...
function Plans() {
  const { data: plans, refetch } = useQuery(getPlansControllerFindAllQueryOptions())
  ...
  const form = useForm({
    ...
    onSubmit: async ({ value }) => {
      try {
        ...
        setSubmitError(err?.response?.data?.message || err?.message || 'Failed to create plan')
  ...
  return (
    <div className="flex flex-col gap-6">
      <div className="flex justify-between items-end border-b-2 border-white pb-4">
        <div>
          <h1 className="text-3xl font-black font-mono uppercase tracking-tighter text-white">Target_Plans</h1>
          <p className="text-muted-foreground font-mono text-xs uppercase tracking-widest mt-1">Manage dietary constraints</p>
        </div>
        <FormDialog
          open={dialogOpen}
          ...
          trigger={<Button className="brutal-border hover:bg-primary font-mono uppercase font-bold rounded-none"><PlusIcon className="w-4 h-4 mr-2"/>New Plan_</Button>}
        >
          <form
            onSubmit={(e) => {
              e.preventDefault()
          ...
          >
            {submitError && (
               <div className="brutal-border bg-red-950 text-red-500 p-3 flex flex-col gap-1">
                  <span className="font-bold flex items-center gap-2"><AlertTriangle size={16} /> ERROR: {submitError}</span>
                  {submitError.includes('missing') && (
                    <span className="text-white mt-2 normal-case">
                      Action required:{' '}
                      <Link to="/settings" className="font-bold underline hover:text-primary transition-colors text-primary" onClick={() => setDialogOpen(false)}>
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              />
              <Label htmlFor="autoCalc" className="cursor-pointer">Enable auto-calculation based on Goal</Label>
            </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {plans?.map((plan) => (
          <Card key={plan.id} className="brutal-border brutal-shadow rounded-none bg-black hover:bg-neutral-900 transition-colors">
            ...
            <CardContent className="pt-6">
              <div className="grid grid-cols-2 gap-4 font-mono text-sm uppercase">
                ...
                <div className="flex flex-col">
                  <span className="text-muted-foreground text-xs">Plan Phase</span>
